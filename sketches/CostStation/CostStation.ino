...
void reconnect() {
  while (!client.connected()) {
    ...
    if (client.connect("costStation")) {
      Serial.println("connected");
      client.subscribe("station/cost"); // change topic name for correct station!!
    } else {
...
void rfidAction() {
  ...
  if (activeCard != 0) {
    if (!isCardStillPresent()) {
      ...
      client.publish("station/cost", msg.c_str()); // change topic name for correct station!!
      strip.clear();
      strip.show();
      activeCard = 0;
    } else {
